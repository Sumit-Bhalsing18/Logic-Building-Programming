...
void ManPageDisplay(char Name[])
{
    ...
    else if(strcmp(Name,"stat") == 0)
    {
        ...
        printf("File_Name : Name of file whose information should be fetch \n");
    }
    else
    {
...
{
    PINODE temp = NULL; //for treversal 
   ...
   while(temp != NULL)
   {
      if(strcmp(temp->FileName, name) == 0)    //inode ch name ani tunhi dilyal same age 
      {
            printf("-----------------------------------------------\n");
           ...
           {
               printf("File Permission is read + write\n");
           }
           Type = temp->FileType;
...
int write_file(int fd ,char* data ,     int Size)
{
   ...
   uareaobj.UFDT[fd] ->ptrinode->ActualFileSize = uareaobj.UFDT[fd]->ptrinode->ActualFileSize+
    return 0;
}
...
//  Description :       It is used to read
//                      the data into specific file
...
int main()
{
    ...
    while(1)
    {
        ...
        else if(iCount == 2)
        {
            ...
            // Marvellous CVFS : > write 1  ha fd ahe
