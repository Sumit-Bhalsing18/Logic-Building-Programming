...
struct Inode
{
    ...
    int ReferenceCount;         //एखाद्या INODE ला किती File Names (Links) आहेत ते LinkCount सांगतो.
    int Permission;
    char *Buffer;               //buffer store address of heap memeory it is pointer point ti heap memery and file memory of heap store actual data of file  
    struct Inode *next;
};
...
{
    char ProcessName[20];
    PFILETABLE UFDT[MAXOPENFILES];   //UFDT म्हणजे Pointers चा Array./*PFILETABLE आधी का लिहिल कारण UFDT मध्ये FILETABLE Structure नाही, तर FILETABLE चा Pointer ठेवायचा आहे. */
};
