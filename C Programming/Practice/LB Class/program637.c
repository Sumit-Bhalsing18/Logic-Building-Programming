...
int main()
{
    char str[80] = {'\0'};           //for input
    char Command[5][20] = {{'\0'}};  //tokenization 
    int iCount = 0 , iRet = 0;

    StartAuxillaryDataInitialisation();
    printf("----------------------------------------------------\n");
    printf("--------Marvelous CVFS started succesfully----------\n");
    printf("----------------------------------------------------\n");

    //Infinite listening Shell

    while(1)  //shell started
    {
       fflush(stdin);//june keyboard buffer dhutle
       strcpy(str,""); //second time la clean bhetla pajihe mhnun

       printf("\n Marvellous CVFS : >");
       fgets(str,sizeof(str),stdin); //input ghetl
       ...
       if(iCount == 1)
       {
          ...
          {
             printf("Thannk you for using Marvellous CVFS \n");
             printf("Deallocating all resource of Marvelous Cvfs \n");
