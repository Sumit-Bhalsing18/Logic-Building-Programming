...
void Display(PNODE first , PNODE last)       //1 pointer vadhla karan circle ahe null nahi kalnar nahi kuth thambaychay
{
   ...
   //jar first ani tail NULL asel tar NULL cha data print hoil je ki garbage yeyil mhnun filter lav 
   //ISSUE ahe karan apn main madhn NULL astani head ani tail apn call karat ahe 
   do
   {
      printf(" | %d | ->" ,first->data);
      first= first->next;
...
int main()
{
    ...
    Display(head ,tail);   // doni pn null asel tar te 
